package com.yimeng.common.luban;
...
import static com.yimeng.common.luban.Preconditions.checkNotNull;
...
public class Luban {
    ...
    public Luban loadList(List<File> fileLists) {
        mFileList = fileLists;
        return this;
    }
    ...
    public Observable<File> asListObservable() {
        if (gear == FIRST_GEAR)
            return Observable.from(mFileList).map(new Func1<File, File>() {
                @Override
                public File call(File file) {
                    return firstCompress(file);
                }
